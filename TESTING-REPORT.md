...
**One abuse/cost-risk item worth flagging as urgent-ish, though it's not a data-exposure vulnerability:** BUG-03 below — `/api/feedback` has no rate limiting beyond an easily-bypassed honeypot, so it could be spammed to run up your Resend usage and flood your inbox. Not a breach risk, but worth prioritizing if this project gets real traffic soon.
...
| BUG-02 | 1/7 | Major | Service worker caches `index.html` cache-first, so returning visitors can get a stale page referencing deleted (content-hashed) JS/CSS after a new deploy, breaking the app until a second reload | Install/visit the PWA, let a new deploy ship (asset filenames change every build since they're content-hashed), revisit without a hard refresh. `sw.js`'s fetch handler does `return cached \|\| network` for every same-origin GET including navigations, so a cached stale `index.html` referencing e.g. `index-DsVtnWah.js` is served even though only `index-CFCRkL64.js` exists post-deploy → `net::ERR_FAILED` loading the script, blank page. Reproduced live: a tab that had an old cached page hit exactly this. | Open — needs a strategy decision (network-first for navigations / HTML, cache-first only for hashed assets), flagged rather than changed since it's a real behavioral change to confirm first |
| BUG-03 | 4 | Major (abuse/cost risk, not a data-exposure vulnerability) | `/api/feedback` has no rate limiting — only a honeypot field, which is trivial to bypass by anyone reading the (now-public) source | Send repeated valid POST requests to `/api/feedback` with `website` left empty. Nothing throttles the request rate. Each one triggers a real Resend API call and a real email to chef@designpav.in. Since the source is public on GitHub, the honeypot field name (`website`) is visible to anyone. | Open — flagged for a decision, not fixed. A proper fix needs a persistent store for request counts (e.g. Vercel KV/Upstash) since serverless functions don't share in-memory state between invocations; that's more than a "simple, low-risk" change, so I didn't implement it without checking first. |
...
| BUG-07 | 6 | Major (accessibility) | The page's `<meta name="viewport">` sets `user-scalable="no"` and `maximum-scale="1.0"`, which disables pinch-to-zoom on the page/UI text for every visitor — a direct WCAG 1.4.4/1.4.10 violation that specifically blocks low-vision users from zooming | Lighthouse `meta-viewport` audit on `/` scored 0. Confirmed in `index.html`'s viewport meta tag. | **Not changed** — this looks like a deliberate choice (likely to stop the browser's own pinch-zoom from fighting the canvas's own pan/zoom gestures on touch devices), so it's a real product tradeoff, not a simple fix. Flagging for a decision rather than unilaterally removing it. |
...
**🟠 Major — fix before wide publicity:**
1. **BUG-01** — No 404/catch-all route; mistyped or broken URLs show a blank white page.
2. **BUG-02** — Service worker caches `index.html` cache-first; returning visitors can hit a stale page referencing deleted assets after any deploy.
3. **BUG-04 / BUG-05** — Toolbar has no responsive layout at all; overlaps below ~860px width, and ~4 tool icons are fully unreachable at 375px phone width. This is probably the single highest-impact item if mobile visitors are expected.
4. **BUG-03** — No rate limiting on the feedback endpoint (abuse/cost risk, not a data vulnerability).
5. **BUG-07** — Viewport meta tag disables pinch-zoom for everyone, a real accessibility violation — needs your call on whether the touch-gesture tradeoff is worth it.

**🟡 Minor:**
6. **BUG-06** — No pinch-zoom/two-finger-pan gesture on touch (zoom still reachable via +/− buttons).
7. Clipboard-copy-as-PNG couldn't be confirmed in this automated environment (permissions) — needs one manual check in a real browser.
8. Copy/paste-to-system-clipboard and align/distribute for multiple elements don't exist (not documented as existing either, so not bugs — just absent features the generic QA checklist expected).

**⚪ Cosmetic / polish (not filed as bugs):**
...
**✅ Already fixed during this pass** (see FIX-01 through FIX-05 in the bug list): feedback form name/email length caps, primary-button color contrast, command-palette focus indicator, silent failure on non-image upload, negative dimensions from inverted resize drags.

---

## Deliverables summary

1. **This file** (`TESTING-REPORT.md`) is the complete report — bug list with severity/repro steps, phase-by-phase findings, and this punch list.
2. **Punch list**: see above, ordered blocker → major → minor → cosmetic.
3. **Fixes made directly** (all small, low-risk, code-reviewed and type-checked): `api/feedback.ts` (name/email length caps), `src/style.css` (button contrast, command-palette focus ring), `src/components/Toolbar.tsx` (image-upload error handling), `src/canvas/Canvas.tsx` (resize clamp). Nothing else was changed — the responsive-layout redesign, service-worker caching strategy, rate limiting, and the viewport-zoom tradeoff were all left for your decision since they're real product/architecture calls, not simple fixes.
